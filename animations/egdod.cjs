...
drawOutline(image, size, outlineColor, alpha) := (
    // Brute force
    ...
    colorplot(
       ...
       (outlineColor.x, outlineColor.y, outlineColor.z, 1) * vis * alpha;
    );
);
...
segment(p, q, overshoot) := [lerp(p,q,-overshoot), lerp(p,q, 1+overshoot)];
...
cieZ(lambda) := 1.217 * skewGauss(lambda, 437.0, 11.8, 36.0) + 0.681 * skewGauss(lambda, 459.0, 26.0, 13.8);
    ...
    timeOffset(t, a, b) := clamp((t - a) / (b - a), 0, 1);
